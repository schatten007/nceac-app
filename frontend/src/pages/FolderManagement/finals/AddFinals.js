...
function AddFinals() {
  ...
  const addinpdata = async (e) => {
    ...
    var { finalsTitle, questionurl, besturl, averageurl, worsturl, marksurl } =
      inputValue;
    questionurl = questionPath;
    console.log(questionurl);

    const res = await fetch("http://localhost:8003/addfinals", {
    ...
    if (res.status === 422 || !data) {
      alert("Please fill all the fields or  re-check Finals Title");
      console.log("error");
    } else {
      // setCMData(data)
      navigate("/finals");
